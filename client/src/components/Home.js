...
const Home = (props) => {
  ...
  let handleSubmit = function (e) {
    ...
    setSearching(true)
    setPage(0)
    if (filter === 'Dog') props.getRazas(raza.toLowerCase())
    ...
    <div className='homeCointainter'>
      <div className='navbar navbar-expand-lg navbar-light bg-light'>
        ...
        <Link className='btnCreate' to='/creacion'>
          <button className='btn'>Add new Dog</button>
        </Link>
      </div>
        ...
        <div className='cardsBox'>
          {props.razas && props.razas.length > 0 ? props.razas.slice(page * 8, page * 8 + 8).map((e) => {
            ...
            : props.razas && props.razas.length < 1 ? <div className='razaCardNotFound'>
              ...
              <span className='textCardNotFound'>Sorry, but we couldn't find any dog to match your search!</span>
            </div>
              : searching &&
