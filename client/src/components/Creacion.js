...
export function Creacion() {
  ...
  const handleSubmit = async e => {
    e.preventDefault()
    axios.get("http://localhost:3001/dogNames").then(function (response) {
      if (response.data.includes(state.nombre)) {
        setState({
          ...state,
          exists: true
        })
      ...
      <div className='form'>
        ...
        <form onSubmit={(e) => handleSubmit(e)}>
          ...
          <div className='formItem'>
            ...
            <input type="text" onChange={handleChange} value={state.temperamento} name='temperamento' placeholder='temp' required />
            &nbsp;<span>*</span>&nbsp;
            <span >or</span>&nbsp;
          ...
          <div>
            <span className='errorMsg'>{state.exists && <span className='errorText'>A dog with that name already exists</span>}</span>
          </div>
        </form>
      </div>
