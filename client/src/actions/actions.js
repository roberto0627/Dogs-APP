...
export function getId(id) {
  ...
  return async function (dispatch) {
    axios.get("http://localhost:3001/dogs/" + id).then(function (response) {
      return dispatch({ type: GET_ID, payload: response.data });
...
export function getRazas(raza) {
  ...
  return async function (dispatch) {
    ...
    axios.get("http://localhost:3001/dogs?name=" + raza).then(function (response) {
      return dispatch({ type: GET_RAZAS, payload: response.data });
    }).catch(err => console.log(err))
