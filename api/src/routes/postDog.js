...
const postDog = function (req, res) {
  ...
  axios('http://localhost:3001/dogNames').then(resultado => {
    if (resultado.data.includes(nombre)) return res.status(422).send('Dog already exists')
    else {
      ...
      axios('http://localhost:3001/temperament').then(resultado => {
        ...
        Dog.create({ nombre: nombre, peso: peso, altura: altura, vida: vida, imagen: imagen }).then(resultado => {
          temps.forEach(i => {
